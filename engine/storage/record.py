...
class Record:
    ...
    def __init__(self, schema: Schema, values: Sequence[RecordValue]) -> None:
        if not isinstance(schema, Schema):
            raise TypeError("Record schema must be a Schema object")
        if isinstance(values, (str, bytes, bytearray)) or not isinstance(
            values, Sequence
        ):
            raise TypeError("Record values must be a sequence of scalar values")
    ...
    def __getitem__(self, name: str) -> RecordValue:
        """Return a value by exact column name; missing names raise KeyError."""
